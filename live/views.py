...
from learning.models import Class, Student, VocabularyList
...
@login_required
def teacher_live_console(request):
    """Render a simple console page so teachers can launch live sessions."""

    if not getattr(request.user, "is_teacher", False):
        return redirect("teacher_dashboard")

    classes = Class.objects.filter(teachers=request.user).order_by("name")
    vocab_lists = VocabularyList.objects.filter(teacher=request.user).order_by("name")
    recent_sessions = (
        LiveGameSession.objects.filter(host=request.user)
    ...
    context = {
        ...
        "question_time_default": getattr(settings, "QUESTION_TIME_DEFAULT", 20),
        "scoring_modes": LiveGameSession._meta.get_field("scoring_mode").choices,
        "api_base_url": reverse("live-game-list"),
    }

    return render(request, "live/teacher_console.html", context)
