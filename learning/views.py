...
def practice_session(request, vocab_list_id):
    ...
    remaining_key = f"practice_remaining_{vocab_list_id}"
    queue = request.session.get(queue_key, [])
    remaining = request.session.get(remaining_key, [])
    activities = [
        "show_word",
        "flashcard",
        "typing",
    ...
    def _init_queue():
        words = list(get_due_words(student, vocab_list, limit=20))
        active = words[:3]
        rem = [w.id for w in words[3:]]
        request.session[remaining_key] = rem
        return [{"id": w.id, "step": 0} for w in active]
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest" or request.GET.get("next"):
        ...
        if item["step"] + 1 < len(activities):
            ...
            if remaining:
                ...
                queue.append({"id": next_id, "step": 0})

        request.session[queue_key] = queue
        request.session[remaining_key] = remaining
